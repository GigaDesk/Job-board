...
const REQUEST_EMPLOYEE_PASSWORD_RESET_MUTATION = gql(`
...
export default function Detail() {
  ...
  const [requestEmployeePasswordReset, { data, loading, error }] = useMutation(
    REQUEST_EMPLOYEE_PASSWORD_RESET_MUTATION
  );
  ...
  useEffect(() => {
    if (data !== undefined && data !== null) {
      window.localStorage.setItem(
        "AuthenticationToken",
        JSON.stringify(data.requestEmployeePasswordReset)
      );
      router.push(`/employee-sign-in/reset-password`);
    }
  }, [data, router]);
