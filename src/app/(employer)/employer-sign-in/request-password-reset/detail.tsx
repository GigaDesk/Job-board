...
const REQUEST_EMPLOYER_PASSWORD_RESET_MUTATION = gql(`
...
export default function Detail() {
  ...
  const [requestEmployerPasswordReset, { data, loading, error }] = useMutation(
    REQUEST_EMPLOYER_PASSWORD_RESET_MUTATION
  );
  ...
  useEffect(() => {
    if (data !== undefined && data !== null) {
      window.localStorage.setItem(
        "AuthenticationToken",
        JSON.stringify(data.requestEmployerPasswordReset)
      );
      router.push(`/employer-sign-in/reset-password`);
    }
  }, [data, router]);
